...
jpgfile = Image.open('1.png') #import os ; cwd = os.getcwd(); files = os.listdir(cwd) ; print("Files in %r: %s" % (cwd, files)) 

#text = pytesseract.image_to_string(jpgfile)
text = pytesseract.image_to_string(jpgfile) #, (img, lang="bul")
...
with open('OCR _From_1_Picture.txt', 'w') as f: #, encoding="utf-8") for Bulgarian
    print(text, file=f)
